# Sempra Energy — Field Operations Remote Access Guide

**Document Classification:** Internal Use Only
**Department:** IT Operations
**Version:** 2.4
**Last Updated:** May 2026
**Owner:** Sempra Enterprise IT — Field Connectivity Team

---

## 1. Overview and Purpose

This guide provides step-by-step instructions for Sempra Energy field technicians and operations staff who need to access corporate IT systems remotely. Whether you are working from a remote electrical substation, a gas transmission facility, or a temporary field site, this guide covers the tools, procedures, and troubleshooting steps needed to maintain a secure and productive connection to Sempra's enterprise systems.

This guide covers the following systems:

- **Sempra Corporate VPN** (GlobalProtect)
...
| Requirement | Details |
|---|---|
| **Enrolled device** | Your device must be enrolled in Sempra's Microsoft Intune MDM (Mobile Device Management). If your device is not enrolled, contact the IT Operations Helpdesk before your field assignment. |
...
| **Operating System** | Windows 10/11 (latest Intune policy patch applied) or iOS/Android with Sempra Mobile enrolled |
...
| Requirement | Details |
|---|---|
| **Cellular data** | A Sempra-issued mobile hotspot (Cradlepoint IBR900 or equivalent) or a carrier SIM with data enabled |
...
## 3. Connecting to the Sempra Corporate VPN from a Remote Site

Sempra Energy uses **Palo Alto Networks GlobalProtect** as its corporate VPN solution. All access to internal applications, SharePoint, SCADA interfaces, and the OMS from outside a Sempra facility requires an active GlobalProtect VPN connection.
...
3. If prompted for a **Portal Address**, enter: `vpn.sempra.com`
4. Click **Connect**.
5. The **Sign In** screen will appear. Enter your **Sempra email address** (e.g., `jsmith@sempra.com`) and your **network password**.
...
8. You can now access internal Sempra resources.
...
2. Tap **Connect**.
3. If prompted for a portal address, enter: `vpn.sempra.com`
4. Sign in with your Sempra credentials and approve the MFA prompt.
5. The app will display **VPN Connected** once the tunnel is active.

### 3.3 VPN Gateway Selection

GlobalProtect automatically selects the optimal VPN gateway based on your location. Sempra operates VPN gateways in the following regions:

| Gateway | Primary Region |
|---|---|
| `ca-vpn.sempra.com` | California (SDG&E / SoCalGas sites) |
| `tx-vpn.sempra.com` | Texas (Oncor / Sempra Texas sites) |
| `az-vpn.sempra.com` | Arizona and Southwest |
...
The **Sempra Outage Management System (OMS)** is used by field crews and control room operators to view, report, and manage power outages in real time. Field technicians may access OMS in read-only mode from the field to review outage tickets assigned to them.

### 4.1 OMS Web Access

> **VPN Required:** You must be connected to the Sempra GlobalProtect VPN before accessing OMS.

1. Ensure GlobalProtect VPN is connected (see Section 3).
2. Open a supported browser (Microsoft Edge or Chrome).
3. Navigate to: `https://oms.internal.sempra.com`
4. Sign in with your **Sempra network credentials** (same as your email login).
...
The **Sempra Field OMS** mobile app is available for authorized field technicians and is distributed via the **Microsoft Company Portal** app.

1. Open **Microsoft Company Portal** on your device.
2. Locate and install **Sempra Field OMS**.
3. Launch the app and sign in with your Sempra credentials.
4. Approve the MFA prompt.
5. The app will synchronize your assigned outage tickets automatically.

> **Note:** The OMS mobile app requires an active internet connection but does **not** require GlobalProtect VPN — it uses the Sempra Cloud Gateway (Azure AD Application Proxy) for secure access without a full VPN tunnel.
...
> ⚠️ **IMPORTANT — NERC CIP Notice:** SCADA systems at Sempra Energy operate within **NERC CIP-regulated Electronic Security Perimeters (ESPs)**. Remote access to SCADA systems is strictly controlled and audited. Only personnel with a current **CIP-005 Remote Access Authorization** on file with the Cyber Security team may connect remotely to SCADA systems.

### 5.1 Authorized Remote Access Methods

Sempra Energy field personnel with active CIP-005 authorization may access SCADA systems remotely via:

1. **Sempra Secure OT VPN** — a separate VPN tunnel from the standard corporate VPN, used exclusively for OT network access.
...
2. Open the **Sempra OT VPN** client (a separate application from GlobalProtect, labeled "Sempra OT Access").
3. Enter the OT VPN portal address: `ot-vpn.sempra.com`
4. Enter your **OT credentials** (note: these are different from your standard Sempra network credentials).
...
7. Connect to the assigned **Jump Server** address provided by the Cyber Security team (e.g., `jumpserver-west.ot.sempra.com`).
...
| "Invalid username or password" | Verify you are using your Sempra email address and current network password. If your password has expired, use a phone or a site with internet to reset at `https://myaccount.sempra.com`. |
...
| Symptom | Suggested Resolution |
|---|---|
| Cannot sign in to Teams or Outlook | Check that your Sempra account has not been locked. Sign in at `https://myaccount.sempra.com` to verify. |
...
- Consult this guide (Sempra Field Operations Remote Access Guide).
- Check the **Sempra IT Operations SharePoint** site for latest known issues and status updates.
- Try basic troubleshooting (restart device, reconnect VPN, clear browser cache).

### 7.2 Level 2 — IT Operations Helpdesk

- **Phone:** 1-800-SEMPRA-IT (1-800-736-7248) — available 24/7 for field crew
- **Teams Channel:** `#it-field-ops` (if you have Teams connectivity)
- **Email:** `it-helpdesk@sempra.com` (response within 4 hours during business hours)
...
- **Phone:** 1-866-SEMPRA-OT (available 24/7 for OT-related security incidents)
- **Email:** `ot-security@sempra.com`
...
2. **Immediately** contact the Sempra Cyber Security Operations Center (CSOC):
   - **Phone (24/7):** 1-866-SEMPRA-CSOC
   - **Email:** `csoc@sempra.com`
...
5. Document the incident in the **NERC CIP Incident Log** (accessible via the Sempra IT Operations SharePoint when connectivity permits).
...
| Task | How |
|---|---|
| Connect to Corporate VPN | Open GlobalProtect → Connect → `vpn.sempra.com` → MFA approve |
| Access OMS (browser) | VPN connected → `https://oms.internal.sempra.com` |
| Access OMS (mobile) | Company Portal → Sempra Field OMS app |
| SCADA remote access | OT VPN → Jump Server → HMI (authorized personnel only) |
| Reset network password | `https://myaccount.sempra.com` |
| IT Helpdesk (24/7) | 1-800-SEMPRA-IT |
| OT Security (24/7) | 1-866-SEMPRA-OT |
| Cybersecurity Incident | 1-866-SEMPRA-CSOC |
...
Personal devices are not supported for any Sempra remote access. If you are working from a personal device in an emergency, contact the IT Helpdesk for a temporary access exception.

---

*This document is maintained by the Sempra Enterprise IT — Field Connectivity Team. For corrections or updates, contact `it-docs@sempra.com`.*

*© 2026 Sempra. Internal Use Only. Do not distribute outside of Sempra Energy or its subsidiaries.*
