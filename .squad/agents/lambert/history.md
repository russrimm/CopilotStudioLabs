...
- Standardize-all-labs content pass: replaced low-quality placeholder stubs with real, lab-specific content across labs 05, 06, 07, 08, 09, 10, 12, 13, and filled the two validator-flagged gaps in the two Lab 01 variants. For labs 06-10, relocated each lab's existing metadata table under `## Metadata` and wrote 4 real lab-specific objectives. For labs 05/12/13, replaced "Verify that the agent works as expected." with concrete, checkable success criteria. Lab 01-energy-ops-agent: added a `## ✅ Validation — Success Criteria` section tied to the four knowledge sources, grounding/citations, web-search-off, and safety boundaries. Lab 01-intro-workshop: added a proper emoji-prefixed Metadata table plus `## ✅ Validation` and `## ✅ Lab Complete`. Preserved all six `> 🔗 Related lab:` cross-link callouts exactly; no README/script/structure changes. `validate_labs.py` now reports 36/36 PASS. Decision merged to `.squad/decisions.md`; orchestration and session logs written.
